...
export function SkillPreviewPane({
  ...
  const handleCopyRawUrl = useCallback(async () => {
    if (!skillId) return;
    const rawUrl = `https://raw.githubusercontent.com/dianrestu/atomic-router/refs/heads/main/skills/${skillId}/SKILL.md`;
    try {
  ...
  const githubUrl = skillId
    ? `https://github.com/dianrestu/atomic-router/blob/main/skills/${skillId}/SKILL.md`
    : null;
